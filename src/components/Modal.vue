<template>
  <div>
    <div
      :id="props.modalId + '-modalOverlay'"
      class="relative hidden"
      aria-labelledby="modal-title"
      role="dialog"
      aria-modal="true"
    >
      <div
        class="fixed inset-0 bg-neutral-50 dark:bg-neutral-700 bg-opacity-30 dark:bg-opacity-60"
      ></div>
      <div class="fixed inset-0 w-screen overflow-y-auto">
        <div
          class="flex h-screen sm:min-h-screenWH items-end justify-center p-4 text-center sm:items-center sm:p-0"
        >
          <div
            :id="props.modalId"
            :class="props.width"
            class="relative overflow-hidden rounded-lg text-left"
          >
            <!-- Header -->
            <div
              class="flex items-center justify-between p-4 md:p-5 bg-neutral-300 bg-opacity-50 dark:bg-neutral-800 border-b-2 border-neutral-200 dark:border-neutral-700"
            >
              <h3
                class="text-xl font-semibold text-neutral-900 dark:text-white"
                v-text="props.modalTitle"
              ></h3>
              <button
                @click="hideModal(props.modalId)"
                type="button"
                class="text-neutral-900 dark:text-neutral-50 bg-transparent rounded-lg text-lg w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:text-white hover:bg-neutral-300 dark:hover:bg-neutral-600 dark:bg-neutral-700"
                data-modal-hide="default-modal"
              >
                &times;
              </button>
            </div>
            <!-- Header -->

            <!-- Body -->
            <div
              class="bg-neutral-50 dark:bg-neutral-800 p-5 h-fit max-h-[600px] overflow-auto"
            >
              <slot />
            </div>
            <!-- Body -->

            <!-- Footer -->
            <div
              class="border-t-2 bg-neutral-300 bg-opacity-50 dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6"
            >
              <button
                :disabled="!props.actionEnabled"
                @click="props.modalProcess()"
                type="button"
                v-if="props.modalProcess"
                :class="{
                  'ring-danger bg-danger hover:bg-danger-hover':
                    operation == 'delete',
                  'ring-primary bg-primary hover:bg-primary-hover':
                    operation == 'add',
                  'ring-success bg-success hover:bg-success-hover':
                    operation == 'update',
                  'ring-danger bg-danger hover:bg-danger-hover':
                    operation == 'view' && props.actionName == 'Cancel',
                  'opacity-75 cursor-not-allowed': !props.actionEnabled,
                  'opacity-100': props.actionEnabled,
                }"
                class="mt-3 inline-flex w-full justify-center rounded-md px-3 py-2 text-sm font-semibold text-white ring-1 ring-inset sm:mt-0 sm:w-auto sm:ml-2"
              >
                <svg
                  v-if="busy"
                  aria-hidden="true"
                  role="status"
                  class="flex place-self-center w-4 h-4 me-1 text-white animate-spin"
                  viewBox="0 0 100 101"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                    fill="#E5E7EB"
                  />
                  <path
                    d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z"
                    fill="currentColor"
                  />
                </svg>
                {{ props.actionName }}
              </button>
              <button
                v-if="props.showCancel"
                @click="hideModal(props.modalId)"
                type="button"
                class="mt-3 inline-flex w-full justify-center rounded-md bg-secondary hover:bg-secondary-hover px-3 py-2 text-sm font-semibold text-white ring-1 ring-inset ring-secondary-hover sm:mt-0 sm:w-auto"
              >
                Cancel
              </button>
            </div>
            <!-- Footer -->
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup>
import { hideModal } from "../utils";

const props = defineProps({
  modalId: String,
  modalTitle: String,
  operation: String,
  modalProcess: Function,
  actionName: String,
  actionEnabled: {
    default: false,
    type: Boolean,
  },
  showCancel: {
    default: false,
    type: Boolean,
  },
  width: {
    default: "w-1/2",
    type: String,
  },
  busy: {
    default: false,
    type: Boolean,
  },
});
</script>
